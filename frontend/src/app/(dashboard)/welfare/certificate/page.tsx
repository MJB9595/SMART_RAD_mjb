					...
					<table>
						<thead>
							<tr>
								...
								<th style={{textAlign: "right"}}>파일 관리</th>
							</tr>
						</thead>
						<tbody>
							{loading ? (
								...
								<tr className="empty-row"><td colSpan={6}>증명서 발급 신청 내역이 없습니다.</td></tr>
							) : (
								filtered.map((d) => (
									<tr key={d.id}>
										...
										<td style={{textAlign: "right"}}>
											<button className="btn-sm" disabled={d.issueStatus !== "ISSUED"} style={{opacity: d.issueStatus !== "ISSUED" ? 0.5 : 1}}>PDF 다운로드</button>
										</td>
									</tr>
								))
							)}
